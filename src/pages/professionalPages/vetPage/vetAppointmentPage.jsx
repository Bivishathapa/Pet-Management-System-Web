...
import { clearStatusUpdateError } from '../../../feature/setAppointmentStatusVetSlice';
...
export default function VetAppointmentPage() {
  ...
  const { appointments, loading, error } = useSelector((state) => state.vetAppointments);
  const { loading: updateLoading, error: updateError, success: updateSuccess } = useSelector((state) => state.setAppointmentStatusVet);
  const { profile: vetProfile, loading: profileLoading } = useSelector((state) => state.vetProfile);
  ...
  const handleSaveEdit = async (status) => {
    if (!vetProfile?.id) {
      console.error('Vet profile not loaded. Vet ID:', vetProfile?.id);
      alert('Unable to update appointment. Vet profile not loaded. Please refresh the page.');
      return;
    }
  ...
  return (
    <div className="max-w-7xl mx-auto p-6">
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-2xl font-semibold tracking-tight text-slate-900">Veterinary appointments</h1>
      ...
      <div className="bg-white/90 rounded-lg border border-rose-100 p-4 mb-6 shadow-sm shadow-rose-50">
        <div className="max-w-md">
          ...
          <div className="relative">
            <input
              type="text"
              placeholder="Customer, pet, or visit type…"
              value={searchTerm}
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gradient-to-r from-rose-50 to-pink-50 border-b border-rose-100">
                  ...
                  <th className="px-4 py-3 text-left text-xs font-medium text-rose-900/70 uppercase tracking-wide">Visit type</th>
              ...
              <tbody className="divide-y divide-rose-100/70">
                {filtered.length === 0 ? (
                  ...
                  filtered.map((apt, index) => {
                    ...
                    const appointmentType = apt.appointmentType || 'N/A';
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-4 py-3 text-slate-700 capitalize">{appointmentType}</td>
                        ...
                        <td className="px-4 py-3 text-center">
                          <button
                            ...
                            disabled={apt.status === 'completed' || apt.status === 'cancelled' || !vetProfile?.id}
                            className={`inline-flex items-center gap-1 px-2 py-1 rounded-md text-xs font-medium transition-colors ${
                              apt.status === 'completed' || apt.status === 'cancelled' || !vetProfile?.id
                          ...
                          >
                            ...
                            {apt.status === 'completed' || apt.status === 'cancelled' ? 'Locked' : !vetProfile?.id ? 'Loading...' : 'Edit'}
                          </button>
                        </td>
                      </tr>
                    );
